...
#include "../util/params_lilotane.h"
#include "../util/log.h"

/**
 * Taken from Hordesat:ParameterProcessor.h by Tomas Balyo.
 */
void ParametersLilotane::init(int argc, char** argv) {
    setDefaults();
...
void ParametersLilotane::setDefaults() {
    ...
    setParam("ed", "200"); // initial expansion depth for iterative deepening search
    setParam("ef", "1.2"); // expansion factor
}

void ParametersLilotane::printUsage() {
    ...
    Log::i(" -ed=<int>           Starting depth for iterative deepening search (0: infinite, aka DFS)\n");
    Log::i(" -ef=<float>         Expansion factor for increasing IDS (depth = depth * ef)\n");
    Log::i("\n");
    printParams();
    Log::setForcePrint(false);
}

std::string ParametersLilotane::getDomainFilename() {
  return _domain_filename;
}
std::string ParametersLilotane::getProblemFilename() {
  return _problem_filename;
}

void ParametersLilotane::printParams() {
    std::string out = "";
...
void ParametersLilotane::setParam(const char* name) {
    _params[name];
}

void ParametersLilotane::setParam(const char* name, const char* value) {
    _params[name] = value;
}

bool ParametersLilotane::isSet(const std::string& name) const {
    return _params.count(name);
}

bool ParametersLilotane::isNonzero(const std::string& intParamName) const {
    return atoi(_params.at(intParamName).c_str()) != 0;
}

std::string ParametersLilotane::getParam(const std::string& name, const std::string& defaultValue) {
    if (isSet(name)) {
        return _params[name];
    } else {
        return defaultValue;
...
std::string ParametersLilotane::getParam(const std::string& name) {
    return getParam(name, "ndef");
}

int ParametersLilotane::getIntParam(const std::string& name, int defaultValue) {
    if (isSet(name)) {
...
int ParametersLilotane::getIntParam(const std::string& name) {
    assert(isSet(name));
    return atoi(_params[name].c_str());
}

float ParametersLilotane::getFloatParam(const std::string& name, float defaultValue) {
    if (isSet(name)) {
...
float ParametersLilotane::getFloatParam(const std::string& name) {
    assert(isSet(name));
    return atof(_params[name].c_str());
}
