...
Substitution::Substitution(const std::vector<int>& src, const std::vector<int>& dest) {
    ...
    for (size_t i = 0; i < src.size(); i++) {
        if (src[i] != dest[i]) {    // only substitute what will actually change
            assert(!count(src[i]) || (*this)[src[i]] == dest[i]); // there is either no entry for src[i] yet or it is the same as the new entry dest[i]
            add(src[i], dest[i]);   // add a new entry
        }
    }
}
