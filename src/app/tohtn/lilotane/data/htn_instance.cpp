...
HtnInstance::HtnInstance(const std::string& domain_file_name, const std::string& problem_file_name) :
             _p(*parse(domain_file_name, problem_file_name)),
            _share_q_constants(true) {
    ...
    Log::i("Create Actions:\n");
    // Create actions
    for (const task& t : primitive_tasks) {
        createAction(t);
    }

    Log::i("Create reductions:\n");
    // Create reductions
    for (method& method : methods) {
        createReduction(method);
    }

    Log::i("Done extracting info\n");

    primitivizeSimpleReductions();
...
std::vector<int> HtnInstance::convertArguments(int predNameId, const std::vector<std::pair<string, string>>& vars) {
    ...
    for (const auto& var : vars) {
        // use predNameId to keep unique names for parameters with otherwise equal names!
